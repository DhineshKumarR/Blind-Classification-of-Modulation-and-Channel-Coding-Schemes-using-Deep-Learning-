...
clear all
%% load input data%%-------------------------------------------------------
load('traindata_minus1dB_10000.mat');
%%-------------------------------------------------------------------------
%       numFeatures = size(meas, 2); % Total number of features - should be 4
meas = traindata_minus1dB_10000([1:16],:);
meas = meas';
numFeatures =  traindata_minus1dB_10000([1:16],:);
numFeatures = size(numFeatures, 1);
[IDs] =traindata_minus1dB_10000(19,:); % Convert character labels to unique IDs
numClasses = max(IDs); % Get total number of possible classes
M = traindata_minus1dB_10000([1:16],:);
M = size(M, 2); % Number of examples
Y = full(sparse(1 : M, IDs.', 1, M, numClasses)); % Create an output
%%----------neural network------------------------------------
net = NeuralNet2([numFeatures 256 128 numClasses]); % Create Neural Network object
...
net.LearningRate = 0.01;         % Learning rate is set to 0.1
...
%% to test the optimised network with new set of datas
% load('testdata_7dB_100.mat');    %to test load the data      
% test = testdata_7dB_100([1:16],:);  %modelling
% test = (test)';
