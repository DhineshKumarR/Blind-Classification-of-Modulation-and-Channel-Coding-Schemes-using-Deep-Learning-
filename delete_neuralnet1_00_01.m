...
clear all
%% load input data%%-------------------------------------------------------
load('traindata_dc_7dB_10000.mat');
%%-------------------------------------------------------------------------
%       numFeatures = size(meas, 2); % Total number of features - should be 4
meas = traindata_dc_7dB_10000([1:16],:);
meas = meas';
numFeatures =  traindata_dc_7dB_10000([1:16],:);
numFeatures = size(numFeatures, 1);
[IDs] =traindata_dc_7dB_10000(19,:); % Convert character labels to unique IDs
numClasses = max(IDs); % Get total number of possible classes
M = traindata_dc_7dB_10000([1:16],:);
M = size(M, 2); % Number of examples
Y = full(sparse(1 : M, IDs.', 1, M, numClasses)); % Create an output
%%----------neural network------------------------------------
net = NeuralNet2([numFeatures 256 256 256 numClasses]); % Create Neural Network object
