...
load('testdata_minus1dB_100.mat');
test = testdata_minus1dB_100([1:16],:);
test = (test)';
Yraw = net.sim(test);         % Use trained object on original examples
[~, Ypred] = max(Yraw, [], 2); % Determine which class has the largest response per example
id = testdata_minus1dB_100(19,:);
figure(1)
